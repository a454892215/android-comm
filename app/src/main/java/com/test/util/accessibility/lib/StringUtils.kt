...
object StringUtils {
    ...
    fun convertToNormal(language: String?): String {
        if (TextUtils.isEmpty(language)) {
            return ""
        }
        val nfdNormalizedString: String =
            Normalizer.normalize(language, Normalizer.Form.NFD)
        val pattern: Pattern = Pattern.compile("\\p{InCombiningDiacriticalMarks}+")
        val s: String = pattern.matcher(nfdNormalizedString).replaceAll("")
        return s.lowercase().trim { it <= ' ' }
    }
